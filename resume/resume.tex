...
\section{教育背景}
  \resumeSubHeadingListStart
    \resumeEducationHeading
      {中国科学院大学 \ (高能物理研究所)}{2023.09 -- 2026.06}
  ...
  \resumeSubHeadingListStart
    ...
    \resumeResearchHeading
      ...
      \resumeItemListStart
        ...
        \resumeItem{\textbf{【架构设计】}从零搭建框架架构，运用\textbf{面向对象编程}思想将“反应堆能谱 $\to$ IBD 微分截面 $\to$ 沉积能量谱 $\to$ 探测器响应 $\to$ 重建能谱”的多层处理链路抽象为\textbf{可复用、可扩展的模块化类层次}：各物理算子封装为独立组件，经统一接口级联为\textbf{四级张量算子流水线}，显著降低耦合度并便于扩展与测试。}
        \resumeItem{\textbf{【技术亮点】}基于 \textbf{PyTorch autograd} 将整条链路重构为端到端可微的\textbf{张量计算图}，把数十项系统误差统一编码为模型参数，以\textbf{梯度下降（gradient descent）}做 $\chi^2$ 极小化，替代传统数值求导，大幅提升收敛速度与数值稳定性。}
        \resumeItem{\textbf{【性能优化】}基于 Profiling 自顶向下定位性能热点（信号谱计算占 98.6\%），实施三项优化：\textbf{分层缓存}（静态量预计算 + 函数级 memoization，约 3$\times$）、\textbf{带状稀疏矩阵}（CSR 存储，矩阵-向量乘 $O(N^2)\to O(N\cdot w)$，约 1.5$\times$）、\textbf{分块张量化构造}（块宽 512，临时张量驻留 L2 缓存，约 1.5$\times$）。}
        \resumeItem{\textbf{【加速成果】}单次拟合耗时：JUNO only \textbf{110\,s $\to$ 15.2\,s}、JUNO + DYB 联合 \textbf{50\,min $\to$ 5\,min}，整体加速超一个数量级；双精度基准下相对误差 $< 10^{-6}$，5000 组 Toy-MC 收敛成功率达 100\%。}
        \resumeItem{\textbf{【科研成果】}框架支撑 \textbf{500 万次拟合}的大规模计算，成果以\textbf{第一作者}发表于 \textbf{\textit{Eur. Phys. J. C} 85 (2025) 1420}（IF 4.8，中科院二区），被 JUNO 合作组采纳为\textbf{基础分析工具}，并在第 25 届 JUNO 国际合作会议作主题报告。}
      \resumeItemListEnd
    ...
    \resumeResearchHeading
      ...
      \resumeItemListStart
        ...
        \resumeItem{\textbf{【工程实现】}基于\textbf{面向对象的 Python} 构建可配置、可复用的自动化 Toy-MC 流水线，支持万级伪实验、多种分 bin 方案一键切换；以分位数（第 16/84 百分位）非参数化定义偏差指标，规避高斯近似假设。}
        \resumeItem{\textbf{【能谱分 bin 策略】}将重建能谱 bin 宽由 20\,keV 调整至 \textbf{100\,keV}，关键参数统计偏差由 0.8$\sigma$ 降至 $<$ \textbf{0.2$\sigma$}；反应堆中微子能谱分段为300keV时，模型依赖系统误差最小（$<$ \textbf{0.3\%}）—— 被 \textbf{JUNO 首篇物理论文}采纳为分 bin 策略（\textit{Nature}, arXiv:2511.14593）。}
        \resumeItem{\textbf{【Feldman--Cousins 方法工程化】}将 FC 构造分解为 50 网格点 $\times$ 5{,}000 伪实验 $\times$ 20 多起始点 $=$ \textbf{500 万次独立拟合}，在 \textbf{1{,}000+ 核 HPC 集群}上并行编排；针对似然面多极小值采用多起始点全局优化。由经验 $\Delta\chi^2$ 分布提取临界值 \textbf{$\Delta\chi^2 \approx 1.57$}（显著高于渐近近似 1.0），定量证实 Wilks 定理在低统计量下失效，相关统计策略被 \textbf{JUNO 首篇物理论文}采纳（\textit{Nature}, arXiv:2511.14593）。}
      \resumeItemListEnd
    ...
    \resumeResearchHeading
      ...
      \resumeItemListStart
        ...
        \resumeItem{\textbf{【性能剖析】}在\textbf{面向对象 C++} 重建框架中，基于 Linux \texttt{perf} + Hotspot 性能剖析自顶向下定位关键热点函数，针对性重构核心计算模块并实施底层优化。}
        \resumeItem{\textbf{【算法优化】}引入\textbf{稀疏化计算}，按事例信息筛选有效 PMT、跳过无贡献 PMT，避免对全部 PMT 的无效遍历；将与拟合参数无关的计算\textbf{移出 Minuit 似然循环}，预计算后复用，消除迭代中的重复开销。}
        \resumeItem{\textbf{【并行优化】}通过\textbf{多线程并行}与几何计算\textbf{向量化（SIMD）}进一步压缩单次重建耗时。}
        \resumeItem{\textbf{【加速成果】}单次事例重建\textbf{速度提升 50\%}，并在第 26 届 JUNO 国际合作会议作主题报告。}
      \resumeItemListEnd
