...
分块计算方法在实践中带来了可观的性能提升。为了确定块宽的最优取值并避免过度调参，本文在测试平台（Intel Xeon Gold 6338）上对若干候选值（128、256、512、1024）进行了基准测试。结果表明，当块宽设置为 512 时，探测器响应矩阵的构造时间从约 60~ms 降至约 27~ms（加速比约 $2\times$）。

针对此类 CPU Cache 层级的优化，本文进一步探讨了手动分块与依赖现代底层库自动优化之间的界限。在传统的矩阵乘法等 BLAS 操作中，底层代码通常会自动利用 OpenMP 进行多线程调度与自动分块（Auto-blocking）。然而，对于涉及 $\operatorname{erf}$ 等复杂特殊函数的逐元素（Element-wise）复合操作，主流深度学习框架（如 PyTorch）对中间临时张量的显存/内存管理并未激进地应用自动缓存分块。本文测试表明，若采用 NumPy/SciPy 路径，底层完善的 C++ 绑定使得手动分块与否的差异小于 5\%，此时手动分块确属过度工程；但在 PyTorch 路径下，手动分块能带来 30\% 到 35\% 的显著加速。这主要是因为分块减小了临时张量的尺寸（如从 25~MB 降至 1~MB 级别），使其能够完整驻留在高速缓存（如 L2 缓存）中，充分利用了数据局部性（Data locality），避免了海量数据超出缓存带来的命中率下降。代表性结果列于表~\ref{tab:block_backend_compare}。

\begin{table}[htbp]
\centering
\bicaption{四种实现下探测器响应矩阵（$560\times 5600$）单次构造耗时（单位：ms）。PyTorch 限定为单线程；\texttt{full} 表示一次性构造整个矩阵。}{Wall-clock time (ms) for constructing the $565\times 5600$ detector response matrix under four implementations. PyTorch is restricted to a single thread; \texttt{full} denotes single-shot construction.}
\label{tab:block_backend_compare}
\begin{tabular}{lcc}
\toprule
配置 & PyTorch & NumPy + SciPy \\
\midrule
chunked, block~=~128  & 34.89 & 49.46 \\
chunked, block~=~256  & 20.52 & 47.60 \\
chunked, block~=~512  & 27.37 & 47.49 \\
chunked, block~=~1024 & 20.62 & 47.77 \\
chunked, block~=~2048 & 35.70 & 52.94 \\
full single-shot      & 63.40 & 47.23 \\
\bottomrule
\end{tabular}
\end{table}
...
为响应对本框架在不同输入条件下稳健性的关注，针对 30~天、50~天及 100~天三个统计量水平开展拟合收敛性测试，采用 JUNO+Daya~Bay 联合拟合，利用 Daya~Bay 近点探测器数据约束反应堆中微子能谱。拟合过程通过 Python 接口调用 Minuit2 工业级优化库实现。收敛判定以 \texttt{Minuit.is\_valid} 为准，确保 MIGRAD 算法寻优成功、预期极小值距离（EDM）低于阈值且 Hesse 矩阵正定（无奇异性）。
在每个统计量下，分别独立生成 5000 组 Toy-MC 伪实验样本进行拟合验证。表~\ref{tab:convergence} 汇总了三个不同统计量下的拟合收敛概率：100~天统计量下成功率达到 100\%；50~天下成功率为 99.85\%；即便在低统计量、统计涨落剧烈的 30~天初期场景下，成功率仍达到 99.18\%（拟合失败率低于 1\%）。上述结果表明，本加速框架在覆盖反应堆中微子分析早期到中期典型曝光区间内，均能保持极高的数值稳定性与收敛性，对反应堆能谱形状的具体输入条件不敏感。

\begin{table}[htbp]
\centering
\bicaption{不同统计量水平下 JUNO+Daya~Bay 联合拟合的收敛性测试结果。每个统计量下独立生成 5000 组 Toy-MC 伪实验，统计拟合收敛的概率。}{Convergence test of the JUNO+Daya~Bay joint fit at three exposure levels. For each level 5000 independent Toy-MC pseudo-experiments are generated, and the success and failure rates returned by the minimizer are tabulated.}
...
本章所提出的高性能前向折叠框架，虽然以中基线反应堆中微子振荡分析为主要应用场景，但其核心设计逻辑具备良好的普适性。本加速框架的核心组件——张量化前向折叠、微分散射截面与响应矩阵的稀疏化、静态量预计算与函数级缓存、以及响应矩阵的分块构造——均不依赖于 JUNO 探测器的特定几何或反应堆类型，仅依赖于"真能谱→响应核→重建谱"这一链式生成结构。因此，对于其他低统计量数据分析场景，本框架可直接迁移使用，典型适用于超新星中微子监测、无中微子双贝塔衰变（$0\nu\beta\beta$）搜索以及暗物质直接探测等背景驱动型（Background-limited）稀疏信号实验等。
