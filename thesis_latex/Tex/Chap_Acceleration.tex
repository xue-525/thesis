...
\begin{table}[htbp]
\centering
\bicaption{四种实现下探测器响应矩阵（$565\times 5600$）单次构造耗时（单位：ms）。PyTorch 限定为单线程；\texttt{full} 表示一次性构造整个矩阵。}{Wall-clock time (ms) for constructing the $565\times 5600$ detector response matrix under four implementations. PyTorch is restricted to a single thread; \texttt{full} denotes single-shot construction.}
