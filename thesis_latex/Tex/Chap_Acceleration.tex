...
\paragraph*{方法论选择：前向折叠（Forward Folding）与反卷积（Unfolding）}\mbox{}\\
中微子振荡分析在处理"探测器重建能谱 $\leftrightarrow$ 反应堆中微子能谱"这一层对应关系时，原则上有两种互为逆过程的方法论\cite{Cowan:2010js, Canelli2025}：一是\textbf{前向折叠}（Forward Folding），即保持"理论真能谱 $\to$ IBD 截面 $\to$ 沉积能谱 $\to$ 探测器响应 $\to$ 该方法遵循物理过程的因果链条，即“理论能谱 $\to$ IBD 截面 $\to$ 沉积能谱 $\to$ 探测器响应 $\to$ 重建能谱”。在重建能量空间构造预测谱并与观测数据比对，通过最小化 $\chi^2$ 或极大似然估计来提取物理参数。二是\textbf{反卷积}（Unfolding / Deconvolution），该方法试图通过对观测谱 $\boldsymbol{M}$ 应用响应矩阵 $R$ 的某种正则化逆（如迭代 Bayesian unfolding 或 SVD unfolding），估计出“去仪器效应”的等效真能谱 $\hat{\boldsymbol{\Phi}}$，再在真能谱层面与理论预测进行比较~\cite{An2021Unfolding, Canelli2025}。

两种方法在物理严谨性与计算特征上存在显著差异。在物理严谨性方面，前向折叠具有决定性优势。由于响应矩阵 $R$ 通常是病态（ill-conditioned）算子，直接求逆会剧烈放大重建谱上的统计涨落，并产生显著的 bin 间负相关结构。为抑制此类非物理振荡，解卷积必须引入正则化（Regularization）平滑假设，而正则化强度的选取往往引入难以量化的先验依赖。相比之下，前向折叠始终保持前向概率映射的清晰性，统计误差传递遵循原生的泊松/高斯分布，系统误差通过冗余参数（nuisance parameters）自然嵌入目标函数中（如式~\eqref{eq:Chisq:Total}），不会破坏原始的协方差结构。这对于追求亚百分比精度的 JUNO 实验而言，是保证统计结果稳健性的核心前提。在计算特征方面，解卷积方法的单次拟合计算开销较低，因为其核心的矩阵反演或正则化过程通常只需执行一次（或少量迭代）。然而，前向折叠的计算开销则随着拟合迭代次数呈线性增长。在每一次拟合步长中，程序必须针对更新的振荡参数与冗余参数重新执行全流程的能谱构造。尤其在进行 Feldman-Cousins 频繁测研究（涉及 $10^5 \sim 10^6$ 量级的伪实验拟合）时，能谱构造涉及的大规模矩阵乘法与高频误差函数（$\operatorname{erf}$）评估会累积出巨大的计算负担。

综合以上方法论比较，为了在保持前向折叠物理严谨性的同时，克服其在计算量上的固有劣势，本章提出了一套高性能的拟合加速方案。通过张量化重构、稀疏矩阵优化及缓存预计算策略，显著降低了单次前向折叠的计算耗时，使大规模统计学验证在有限算力下成为可能。
...
\end{itemize}
\paragraph{优化策略的权衡：领域逻辑引导与编译器自动优化}\mbox{}\\
在高性能计算任务的工程实现中，明确手动优化与现代编译器（如 GCC/Clang \texttt{-O3} 级别）自动优化之间的界限，是规避“过度工程化”风险的关键。现代编译器及高性能框架（如 PyTorch）的运行时系统已在指令重排、寄存器分配及基础向量化方面展现出卓越的自动化处理能力。本工作的优化重心并未试图介入上述底层通用调度，核心优化策略在于利用 Python/PyTorch 生态中成熟的 \texttt{lru\_cache} 机制，针对计算密集型函数（如响应矩阵折叠、$\operatorname{erf}$ 批量求值）实施函数级结果复用。由于 \texttt{lru\_cache} 内部基于哈希表实现，其缓存命中严格依赖于输入参数的二进制等价性（Bit-wise identity）。这种方法以极低的代码修改实现了约 $3\times$ 的整体吞吐量提升。
\paragraph{收益与可复用性}
...
\paragraph*{手动分块与编译器自动优化的权衡}\mbox{}\\
这里有必要说明：现代编译器（如 GCC 13 / Clang 17 在 \texttt{-O3 -march=native} 下）对简单的稠密矩阵循环已经具备较强的自动向量化（auto-vectorization）、循环分块（loop tiling）与预取（prefetching）能力，因此对教科书式的稠密 GEMM 而言，手动分块往往相对于已经高度调优的 BLAS 实现并不会带来额外收益，反而可能因削弱编译器的整体视野而降低效率。本工作的响应矩阵构造在三个维度上均超出了编译器自动优化的有效范围：第一，循环体内含误差函数 $\operatorname{erf}$ 这一特殊函数调用，编译器通常无法对其内联展开，因而无法执行跨整行的向量化；第二，构造过程依赖于按列累加的依赖链，循环不可自由重排，自动分块启发式（如 Polly、loop-nest optimizer）在本场景下往往选择不进行分块；第三，我们的实现基于 PyTorch 动态图，底层张量算子由框架在运行时分发，编译器在 Python 侧看不到完整循环结构，亦无从施加循环级优化。基于上述三点，我们选择仅在编译器无法有效向量化的复杂张量收缩环节（即响应矩阵的 $\operatorname{erf}$ 批量求值与按列累加）进行手动分块与块宽调优，对其余常规稠密矩阵—向量乘法等编译器已能良好优化的部分则完全交由 PyTorch/BLAS 后端处理，避免过度工程化（over-engineering）。实测中，手动分块相对于直接让 PyTorch 按默认方式构造整张响应矩阵额外带来约 $2\times$ 加速；若在不含 $\operatorname{erf}$ 的纯稠密矩阵乘法环节强行引入同样的手动分块，反而会破坏 BLAS 的并行调度、使该环节耗时上升约 15\%，这一对照实验从反面印证了"仅在编译器盲区手动优化、编译器可见处保持原生调用"的权衡策略的合理性。
...
\subsection{数值稳定性与收敛性验证}
\label{sec:convergence}

为响应对本框架在不同输入条件下稳健性的关注，针对 30~天、50~天及 100~天三个统计量水平开展拟合收敛性测试，采用 JUNO+Daya~Bay 联合拟合，利用 Daya~Bay 近点探测器数据约束反应堆中微子能谱。在每个统计量档位下，分别独立生成 5000 组 Toy-MC 伪实验样本进行拟合验证。表~\ref{tab:convergence} 汇总了三档曝光量下的拟合成功率与失败率：100~天统计量下成功率达到 100\%；50~天下成功率为 99.85\%；即便在低统计量、统计涨落剧烈的 30~天初期场景下，成功率仍达到 99.18\%（拟合失败率低于 1\%）。上述结果表明，本加速框架在覆盖反应堆中微子分析早期到中期典型曝光区间内，均能保持极高的数值稳定性与收敛性，对反应堆能谱形状的具体输入条件不敏感。

\begin{table}[htbp]
\centering
\bicaption{不同统计量水平下 JUNO+Daya~Bay 联合拟合的收敛性测试结果。每档曝光量独立生成 5000 组 Toy-MC 伪实验，统计拟合器返回的成功率与失败率。}{Convergence test of the JUNO+Daya~Bay joint fit at three exposure levels. For each level 5000 independent Toy-MC pseudo-experiments are generated, and the success and failure rates returned by the minimizer are tabulated.}
\label{tab:convergence}
\begin{tabular}{cccc}
\hline
统计量 & Toy-MC 样本数 & 拟合成功率 & 拟合失败率 \\
\hline
30~天  & 5000 & 99.18\%  & 0.82\% \\
50~天  & 5000 & 99.85\%  & 0.15\% \\
100~天 & 5000 & 100.00\% & 0.00\% \\
\hline
\end{tabular}
\end{table}

进一步讨论方法学的可迁移性。本加速框架的核心组件——张量化前向折叠、微分散射截面与响应矩阵的稀疏化、静态量预计算与函数级缓存、以及响应矩阵的分块构造——均不依赖于 JUNO 探测器的特定几何或反应堆类型，仅依赖于"真能谱→响应核→重建谱"这一链式生成结构。因此，对于其他低统计量数据分析场景，本框架可直接迁移使用，典型适用对象包括：TAO 近点反应堆中微子谱拟合、Daya~Bay 末期低统计区间的惰性中微子（sterile neutrino）搜索、地球中微子（Geo-$\nu$）能谱拟合、超新星预警下的快速能谱重建、以及 JUNO–DUNE 联合振荡分析的早期数据期等。30~天/5000~Toy-MC 仍达到 99.18\% 拟合成功率的实测结果，从数值层面直接支撑了该论断——在事例数稀疏、$\chi^2$ 高斯近似在边缘 bin 趋于失效的条件下，本框架仍能稳定收敛，符合低统计量数据分析对数值鲁棒性的核心要求。
