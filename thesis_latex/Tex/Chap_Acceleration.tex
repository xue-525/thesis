...
为响应对本框架在不同输入条件下稳健性的关注，针对 30~天、50~天及 100~天三个统计量水平开展拟合收敛性测试，采用 JUNO+Daya~Bay 联合拟合，利用 Daya~Bay 近点探测器数据约束反应堆中微子能谱。拟合过程通过 Python 接口调用 Minuit2 工业级优化库实现。收敛判定以 Minuit.is_valid 为准，确保 MIGRAD 算法寻优成功、预期极小值距离（EDM）低于阈值且 Hesse 矩阵正定（无奇异性）。
