...
\begin{enumerate}  
\item \textbf{固定反应堆模型（Fixed Reactor Model）：} 假设反应堆中微子能谱固定为名义求和模型（Summation Model，不引入能谱扰动）。在此基准条件下生成伪实验样本并构建经验 $\Delta\chi^2$ 分布。如图~\ref{fc_dchi2_fixed}，在 68.27\% 置信水平下，提取到的经验临界值为：
\[
...
\item \textbf{扰动反应堆模型（Sampled Reactor Model）：} 为考虑反应堆能谱模型可能存在的细微结构偏差，本研究对能谱进行分段扰动（以 20~keV 为步长），构造出一系列带有精细结构涨落的替代能谱（即扰动模型系综，样本数 $N_{\mathrm{toys}}=5000$）。如图~\ref{fc_dchi2_sampled}，在这些随机抽样的扰动模型上重复 FC 过程，得到的经验临界值为：
\[
