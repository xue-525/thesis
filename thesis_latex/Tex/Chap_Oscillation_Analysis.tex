...
在商用轻水反应堆中，释出的电子反中微子主要来自若干主要裂变同位素的 β 衰变。为了构造到达探测器的中微子通量谱，常用两类方法：一种是基于转换法（conversion）得到的经验谱模型（Huber–Müller）；另一种是基于裂变产物$\beta$支级信息的求和法（summation method），后者能够在理论上产生更丰富的谱形细节。本文以 Huber–Müller 模型作为基准反应堆反中微子能谱模型，用以生成参考到达谱并作为后续不确定度评估与比较的参照；而基于求和法产生的能谱则用于不确定度估计与谱形敏感性研究。在后续的不确定度量化与分 bin 策略研究中，将进一步介绍由 summation 法产生的带有精细结构的反应堆反中微子能谱模型。
