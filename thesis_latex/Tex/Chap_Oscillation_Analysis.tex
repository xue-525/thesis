...
\end{equation}
为了精确描述这一能量映射及其角分布权重，本工作采用了 Vogel 与 Beacom 提供的 IBD 截面模型 \cite{Vogel1999Angular}。该模型包含了质子反冲的一阶修正及完整的角依赖项，能够准确捕捉正电子能谱随角度的微弱偏移以及低能区的动力学展宽效应。在数值实现上，为了处理式~\eqref{eq:continuous_rate} 中的积分，我们将上述连续物理过程离散化为响应矩阵 $M_{ij}$。通过对每一对样本点 $(E_{\bar{\nu}_e}, \cos\theta)$ 计算对应的沉积能量并按微分截面 $\mathrm{d}\sigma/\mathrm{d}\cos\theta$ 加权，该矩阵将入射中微子能谱箱映射为探测器的沉积能量箱。$M_{ij}$ 不仅包含了 IBD 截面的能量依赖性，更忠实地反映了由角分布和反冲效应引入的动力学展宽，是连接理论通量模型与可观测能谱的核心数值单元。
