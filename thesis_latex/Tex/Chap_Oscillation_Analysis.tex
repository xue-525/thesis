...
其中：
$T_{\text{DAQ}}$ 表示数据采集（Data Acquisition，DAQ）时间窗口，对时间 $t$ 的积分即在该窗口内对反应堆通量的瞬时值进行时间积分；$\Phi(E_{\bar{\nu}_e}, t)$ 为 $t$ 时刻在探测器处、经振荡后的反应堆反中微子总到达通量（各堆芯累加，约定见式~\eqref{eq:Phi-total-reactor-flux}），其时间依赖性反映反应堆热功率、各同位素裂变份额及停堆/换料等运行状态的时变特征。$E_{\text{dep}} = E_{\text{dep}}(E_{\bar{\nu}_e}, \cos\theta)$ 表示由中微子能量和散射角确定的沉积能量；$N_p$ 是靶质子数；$\epsilon(E_{\text{dep}})$ 是探测器对沉积能量的效率；$\sigma_{E_{\text{vis}}}$ 是探测器能量分辨。

为了将反应堆产生的反中微子能谱转化为实验中观测到的重建能谱，需要经过时间累积与物理响应卷积的综合处理。这一过程在数学上体现为式~\eqref{eq:phi-Erec-integral} 所示的三重积分，其物理逻辑可解构为以下三个层次：时间维度的累积（Temporal Accumulation）：考虑到中微子实验的长周期运行特征，探测器观测到的是在整个取数周期 $T_{\text{DAQ}}$ 内事例的累积效应。由于反应堆的热功率、燃料组分占比（如燃料演化效应）以及停堆状态随时间动态变化，积分项 $\int \mathrm{d}t$ 确保了理论模型能够准确覆盖这些随时间演化的通量特征。相互作用的能谱转化（Physics Interaction）：反中微子在探测器中与靶质子发生反应，按照微分散射截面 $\frac{\mathrm{d}\sigma}{\mathrm{d}\cos\theta}$ 分布产生沉积能量。该过程通过对中微子能量 $E_{\bar{\nu}_e}$ 和散射角 $\cos\theta$ 的积分，描述了从真能谱到沉积能谱的物理映射，并包含了靶粒子数 $N_p$ 及探测器效率 $\epsilon(E_{\text{dep}})$ 的影响。探测器响应的卷积（Detector Response）：沉积能谱进一步与探测器的能量响应函数（近似为高斯分布）进行卷积。这一步通过对沉积能量 $E_{\text{dep}}$ 的映射，将微观物理过程转化为实验室观测到的重建能量 $E_{\text{rec}}$。式~\eqref{eq:phi-Erec-integral} 通过时间、能量、散射角的三重积分，完整刻画了从中微子源产生到探测器信号输出的全物理过程折叠关系。
...
\end{equation}
其中 $W_r(t)$ 为第 $r$ 个堆芯在时刻 $t$ 的热功率，$f_{ir}(t)$ 表示在时刻 $t$ 第 $r$ 个堆芯中第 $i$ 种裂变同位素的裂变份额（例如 $^{235}\mathrm{U}$、$^{238}\mathrm{U}$、$^{239}\mathrm{Pu}$、$^{241}\mathrm{Pu}$），$e_i$ 为第 $i$ 种裂变同位素每次裂变释放的平均能量，$s_i(E_{\bar{\nu}_e})$ 为第 $i$ 种裂变同位素每次裂变产生的反中微子能谱，其基准形状由 Huber--Müller 转换法\cite{Huber2011Reactor,Mueller2011Improved} 给出，或由基于裂变产物 $\beta$ 支级信息的求和法\cite{Fallot2012_Summation,Estienne2019Summation} 计算。将各堆芯的到达谱按距离衰减与振荡概率叠加，可得探测器处的总到达通量：
\begin{equation}
...
将式~\eqref{eq:phi-r-tot} 给出的修正后发射谱 $\phi_r^{\rm tot}(E_{\bar{\nu}_e},t)$ 代入式~\eqref{eq:Phi-total-reactor-flux}，即可得到探测器处按全部堆芯累加后的到达通量：式~\eqref{eq:Phi-total-reactor-flux} 中的几何衰减因子 $1/(4\pi L_r^2)$ 反映中微子在自由传播过程中的立体角稀释，$L_r$ 为第 $r$ 个堆芯到 JUNO 探测器的基线长度；存活概率 $P_{\bar{\nu}_e\to\bar{\nu}_e}(E_{\bar{\nu}_e},L_r)$ 则描述由三味中微子振荡引起的通量重分布。此处中微子在物质中的传播还受到物质效应的影响，这由 Wolfenstein 最早提出~\cite{Wolfenstein1978}，并由 Mikheyev 和 Smirnov 进一步发展~\cite{Mikheyev1985}。因此 $P_{\bar{\nu}_e\to\bar{\nu}_e}$ 包含了由中微子与地壳电子相互作用所引起的物质效应~\cite{Akhmedov2004}。
...
综上所述，本文在 Huber-Mueller 理论模型的基础上，引入了乏燃料效应与非平衡修正，并采纳大亚湾实验观测到的反应堆能谱畸变作为经验修正因子（即 5 MeV “Bump” 修正），从而得到了更加精确的反应堆出射反中微子能谱 $\phi_r^{\rm tot}(E_{\bar{\nu}_e},t)$（见式~\eqref{eq:phi-r-tot}）。如图~\ref{fig:reactor_nu_arrival} 所示，该谱线为考虑裂变产物随时间演化加权后的结果，作为后续计算到达谱与振荡折算的基准输入。进而结合恒定密度近似下的三味中微子振荡存活概率 $P_{\bar{\nu}_e\to\bar{\nu}_e}(E_{\bar{\nu}_e}, L_r)$，即可求得探测器处由第 $r$ 个堆芯贡献的到达能谱 $\Phi_r(E_{\bar{\nu}_e},t)\equiv \phi_r^{\rm tot}(E_{\bar{\nu}_e},t)\,P_{\bar{\nu}_e\to\bar{\nu}_e}/(4\pi L_r^2)$，按各堆芯累加即得 $\Phi(E_{\bar{\nu}_e},t)=\sum_r \Phi_r$。
...
\paragraph{连续能谱的卷积描述。}
在理想探测器中，观测到的沉积能量谱 $\phi_{\rm dep}(E_{\rm dep})$ 是入射反电子中微子在探测器处的到达通量 $\Phi(E_{\bar{\nu}_e})$ 与 IBD 反应截面在能量空间上的卷积（符号 $\Phi$ 延续式~\eqref{eq:Phi-total-reactor-flux} 对各反应堆堆芯累加后的到达通量约定）。其物理过程可表述为：
\begin{equation}
\label{eq:continuous_rate}
\phi_{\rm dep}(E_{\rm dep}) = N_p \int_{1.8}^{13} \mathrm{d}E_{\bar{\nu}_e} \cdot \Phi(E_{\bar{\nu}_e}) \cdot \frac{\mathrm{d}\sigma}{\mathrm{d}E_{\rm dep}}(E_{\bar{\nu}_e}, E_{\rm dep})
\end{equation}
其中 $N_p$ 为探测器中参与反应的自由质子数目，$\Phi(E_{\bar{\nu}_e})$ 表示入射反应堆反电子中微子在探测器处的到达通量。积分下限 $1.8\,\mathrm{MeV}$ 对应逆贝塔衰变反应的动力学阈值，而上限取 $13\,\mathrm{MeV}$ 以覆盖反应堆反中微子的有效能量范围。沉积能量微分截面 $\mathrm{d}\sigma/\mathrm{d}E_{\rm dep}$ 刻画了给定入射能量 $E_{\bar{\nu}_e}$ 的中微子产生特定沉积能量 $E_{\rm dep}$ 的概率。由于 IBD 反应中正电子的出射具有角分布特征，且质子反冲效应会导致能量分配随角度变化，因此 $\mathrm{d}\sigma/\mathrm{d}E_{\rm dep}$ 需通过对发射角 $\cos\theta$ 积分获得：
\begin{equation}
\frac{\mathrm{d}\sigma}{\mathrm{d}E_{\rm dep}}(E_{\bar{\nu}_e}, E_{\rm dep}) = \int_{-1}^{1} \mathrm{d}\cos\theta \cdot \frac{\mathrm{d}\sigma}{\mathrm{d}\cos\theta}(E_{\bar{\nu}_e}, \cos\theta) \cdot \delta\bigl(E_{\rm dep} - \mathcal{E}(E_{\bar{\nu}_e}, \cos\theta)\bigr)
\label{eq:dsigma-dEdep}
\end{equation}
在此映射关系中，$\mathcal{E}(E_{\bar{\nu}_e}, \cos\theta)$ 代表由动力学确定的沉积能量。对所有可能的发射角按角分布权重（由微分截面 $\mathrm{d}\sigma/\mathrm{d}\cos\theta$ 给出）进行积分，$\delta$ 函数保证仅在正电子产生的沉积能量恰好等于指定 $E_{\rm dep}$ 时才有贡献；在数值实现时 $\delta$ 函数通常以离散化方式处理：将 $\cos\theta$ 作等距或重要性采样，计算每一采样点对应的 $E_{\rm dep}$，并将该采样点按权重 $\mathrm{d}\sigma/\mathrm{d}\cos\theta$ 累加到落入相应沉积能量区间的响应矩阵单元中。在闪烁体探测器中，正电子在沉积其动能 $T_e$ 后会与电子湮灭，释放两个 $0.511$~MeV 的光子，因此沉积能量定义为：
\begin{equation}
E_{\rm dep}(E_{\bar{\nu}_e}, \cos\theta) = E_{e^+}(E_{\bar{\nu}_e}, \cos\theta) + m_e = T_e(E_{\bar{\nu}_e}, \cos\theta) + 2m_e
\label{eq:Edep-positron}
\end{equation}
为了精确描述这一能量映射及其角分布权重，本工作采用了 Vogel 与 Beacom 提供的 IBD 截面模型 \cite{Vogel1999Neutrino}。该模型包含了质子反冲的一阶修正及完整的角依赖项，能够准确捕捉正电子能谱随角度的微弱偏移以及低能区的动力学展宽效应。在数值实现上，为了处理式~\eqref{eq:continuous_rate} 中的积分，我们将上述连续物理过程离散化为响应矩阵 $M_{ij}$。通过对每一对样本点 $(E_{\bar{\nu}_e}, \cos\theta)$ 计算对应的沉积能量并按微分截面 $\mathrm{d}\sigma/\mathrm{d}\cos\theta$ 加权，该矩阵将入射中微子能谱箱映射为探测器的沉积能量箱。$M_{ij}$ 不仅包含了 IBD 截面的能量依赖性，更忠实地反映了由角分布和反冲效应引入的动力学展宽，是连接理论通量模型与可观测能谱的核心数值单元。
...
在实际计算中，我们通过对入射能量 $E_{\bar{\nu}_e}$ 和正电子发射角 $\cos\theta$ 进行双重采样来构建矩阵。数值上，矩阵元 $M_{ij}$ 可表达为如下求和形式：
\begin{equation}
M_{ij} = \sum_{k \in \text{samples}} \frac{\mathrm{d}\sigma(E_{\bar{\nu}_e,j}, \cos\theta_k)}{\mathrm{d}\cos\theta} \cdot \Delta\cos\theta \cdot \mathbf{1} \bigl( E_{\rm dep}(E_{\bar{\nu}_e,j}, \cos\theta_k) \in [E_{{\rm dep},i}, E_{{\rm dep},i+1}) \bigr)
\label{eq:Mij-sum}
\end{equation}
...
采用上述构造方法得到的响应矩阵自然同时包含了 IBD 微分截面的能量与角度依赖、由发射角和反冲引入的动量学展宽，以及离散化带来的能量混合效应。因此矩阵映射 $\phi_{{\rm dep},i}=\sum_j M_{ij}\,\Phi_j$（其中 $\Phi_j$ 为第 $j$ 个能量 bin 内的到达通量、$\phi_{{\rm dep},i}$ 为第 $i$ 个沉积能量 bin 内的谱值）在数值上忠实再现了从入射反中微子通量到可观测沉积能量谱的完整物理过程。由基准反应堆通量与构建好的 $M_{ij}$ 得到的典型沉积能量谱示例见图~\ref{fig:depo_e_spec}。

下面给出构建响应矩阵 $M_{ij}$ 的伪代码流程示例（无探测器分辨率等效应），步骤清晰地描述了从中微子能量和角度采样到沉积能量归档的过程：
\begin{algorithm}[htbp]
\caption{生成响应矩阵 $M[i][j]$ 的伪代码}
\begin{algorithmic}[1]
\Require 中微子能量 bins $j$（中心能量 $E_{\bar{\nu}_e,j}$，算法内简记为 $E_{\nu_j}$），细分 $E_\nu$ 数 $n_{\text{split}}$，余弦角 bin 宽度 $\Delta\cos\theta$
\Ensure 响应矩阵 $M[i][j]$
...
因此微分散射截面矩阵的构建逻辑是先采样中微子能量和出射角度，计算对应的沉积能量，并将相应的微分截面权重累加到正确的能量 bin 中。这样，矩阵 $M_{ij}$ 自动反映了从入射中微子通量到探测沉积谱的完整物理映射关系。
在构建好响应矩阵 $M_{ij}$ 后，对于任意给定的入射反中微子通量向量 $\boldsymbol{\Phi}$（其元素 $\Phi_j$ 为第 $j$ 个能量 bin 内的到达通量积分），观测到的沉积能量能谱向量 $\boldsymbol{\phi}_{\rm dep}$（其元素为 $\phi_{{\rm dep},i}$）可通过简单的矩阵乘法得到：
\begin{equation}
\phi_{{\rm dep},i} = \sum_j M_{ij}\,\Phi_j,\qquad \boldsymbol{\phi}_{\rm dep} = M\,\boldsymbol{\Phi}.
...
\label{eq:Rij-CDF}
\end{equation}
探测器响应矩阵 $R_{ij}$ 既包含由 $f_{\rm LSNL}$ 引起的能量漂移（mean shift），也包含由能量分辨引起的展宽（smearing），能够准确描述沉积能量向重建能量的转换概率分布。
