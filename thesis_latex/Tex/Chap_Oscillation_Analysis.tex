...
\phi(E_{\rm dep}) = N_p \int_{1.8}^{13} \mathrm{d}E_{\nu} \cdot \phi(E_{\nu}) \cdot \frac{\mathrm{d}\sigma}{\mathrm{d}E_{\rm dep}}(E_{\nu}, E_{\rm dep})
\end{equation}
...
\end{equation}  
函数 $f_{\rm LSNL}(E)$ 随能量变化，涵盖了淬灭（quenching）、Cherenkov 光贡献等效应——探测器收集到的光产额与粒子的初始能量损耗并非严格的线性关系。通常先通过刻度数据确定 $f_{\rm LSNL}$ 的参数与不确定度，得到校正后的期望可见能量记作 $E_{\rm vis}^0$。
可见能量的计算通过下式
\begin{equation}
E_{\rm vis} = E_{\rm dep}\,f_{\rm LSNL}(E_{\rm dep})
\label{eq:Evis-LSNL}
...
\end{equation}
其中第一行为分箱数据项，分母包含统计方差与若干背景谱形不确定度项；第二行及后续项为对各类系统不确定性的 Gaussian 约束（拉项）。相关符号及冗余参数（pull parameters）的含义在下文“数据项符号说明”和“系统不确定性 pull term”两小节中统一列出。
