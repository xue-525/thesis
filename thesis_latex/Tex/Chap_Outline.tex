...
\ForAll{中微子能量 bin $j$}
  ...
  \For{$k = 0$ to $n_{\text{split}} - 1$}
    ...
    \For{$\cos\theta = -1$ to $+1$ step $\Delta\cos\theta$}
      ...
      $T_1 \gets E_e(E_{\nu_k}, \cos\theta) - m_e$
      \State 计算沉积能量：
      $E_{\text{dep}} \gets T_1 + 2m_e$
      \State 查找 $i$ 使得
      $E_{\text{dep}} \in
      [E_{\text{dep},i}, E_{\text{dep},i+1})$

      \If{找到合法的 $i$}
        \State 计算微分截面：
        $\dfrac{d\sigma(E_{\nu_k}, \cos\theta)}{d\cos\theta}$
        \State $\text{weight} \gets
        \dfrac{1}{n_{\text{split}}
        } \cdot
        \dfrac{d\sigma}{d\cos\theta}
        \cdot \Delta\cos\theta$
        \State $M[i][j] \gets M[i][j] + \text{weight}$
      \EndIf
    \EndFor
  \EndFor
\EndFor
...
%=====================================================================
\chapter{反应堆拟合程序的加速（Acceleration Strategies）}
%=====================================================================

\textbf{目的}：详述加速方法、并行架构、代码设计与性能。
...
\begin{itemize}
    \item block topology（x × y × z）；
    \item embarrassingly-parallel；
    \item MPI / 多进程 / job farm。
\end{itemize}

\section{Surrogate / ML 加速（GP / Emulator）}
...
\begin{itemize}
    \item speed-up、CPU-hours；
    \item strong/weak scaling；
...
\begin{itemize}
    \item toy-generator, fitter, job-manager, result-aggregator；
    \item config.json、单元测试、CI。
\end{itemize}
