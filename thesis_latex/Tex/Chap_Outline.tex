...
\end{figure}
在低统计量情况下，图~\ref{fig:dmsq31_best_fit} 清楚地表明：来自伪实验的 $\Delta m^2_{31}$ 最优拟合值可以显著偏离输入的真值；且对某一代表性伪实验绘制的 $\chi^2$ 轮廓（右图）表现出高度非二次的行为并包含大量局域极小值。由此可见，在用 Feldman–Cousins（FC）方法构造 $\Delta\chi^2$ 分布时，需要对初始拟合起点采用宽广的扫描区间并使用多个起始值，以保证找到全局最优解，避免拟合陷入局域极小值，从而确保 FC 构造的覆盖率正确。
\begin{figure}[h]
  \begin{subfigure}[t]{0.47\textwidth}
    \centering
