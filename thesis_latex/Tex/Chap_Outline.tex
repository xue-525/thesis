...
中微子的质量来自标准模型之外的机制：由于中微子不带电，其质量项可以是狄拉克型也可以是马约拉纳型。一种被广泛研究的理论是“跷跷板机制”（\textit{see-saw} 机制）：即假定存在标准模型之外的超重右旋（马约拉纳）中微子，通过质能关系使得观测到的轻中微子质量极其微小\cite{Minkowski1977,Yanagida1979,GellMann1979,Mohapatra1980}。这一机制虽然尚未被直接验证，但为中微子质量的自然小值提供了解释。总体而言，中微子质量和振荡现象为粒子物理学揭示了标准模型以外的新物理窗口。这也是迄今唯一一个有坚实实验证据表明需扩展标准模型的现象。未来的大型实验（如无中微子双 $\beta$ 衰变实验）正致力于测量中微子质量本征值、判断中微子究竟是狄拉克还是马约拉纳粒子，并通过精确测定中微子振荡参数来进一步检验混合矩阵的结构和可能存在的非标准相互作用。
...
对于中微子混合矩阵（PMNS 矩阵），中微子混合矩阵 $U_{\mathrm{PMNS}}$ 将味态和质量态联系起来，是一个 $3\times3$ 酉矩阵\cite{Maki1962,Pontecorvo1968}。一般可用三个混合角 $(\theta_{12},\theta_{23},\theta_{13})$ 和一个狄拉克 CP 破坏相位 $\delta$ 来参数化（如果中微子是马约拉纳粒子，还可多出两个马约拉纳相位 $\alpha_{21},\alpha_{31}$）。一种常见的表示形式为：
\begin{equation}
\begin{aligned}
...
由公式~\ref{eq:flavor-mass-superposition} 以及 PMNS 矩阵~\ref{eq:formula_prob} 可推出反应对中微子存活概率为：
\begin{equation}
    \begin{aligned}
    P(\bar{\nu}_e \rightarrow \bar{\nu}_e)
    = \; & 1
...
因此，根据反应堆中微子存活概率的关系如图~\ref{fig:reactor_survival_baseline} 所示，在不同基线条件下，短基线实验（例如大亚湾近/远站）主要观测由 $\Delta m^2_{31}$ / $\Delta m^2_{32}$ 驱动的“快振荡”，其振荡极大值位于约 2\,km 附近；而中长基线实验（例如 JUNO）则位于由 $\Delta m^2_{21}$ 驱动的“慢振荡”极大值附近（约 65\,km），因此 JUNO 能在同一次能谱测量中同时对慢、快两类频率成分敏感。
...
在两味近似下，可以将物质效应的结果写成对混合角与质量平方差的重整化：
\begin{equation}
...
\end{equation}
时，$\theta_m$ 接近 $45^\circ$ 并取得极大值——即所谓的 MSW 共振，此时味态转换被显著增强。值得注意的是，共振条件对中微子与反中微子符号相反（因 $V_e$ 对反中微子取负），因此同一介质下中微子与反中微子的物质修正方向不同。

在三味振荡情形中，需对 $3\times3$ 的有效哈密顿量数值对角化以获得精确的物质修正；一般而言，物质效应主要影响与太阳项（$\Delta m^2_{21}$）相关的低频分量，而对于快频项（由 $\Delta m^2_{3\ell}$ 驱动）影响较小。对于太阳中微子（穿越太阳高密度区域），MSW 效应产生的大幅增强是解释太阳中微子缺失的关键机制；而对于穿过地壳传播的反应堆反中微子（基线为数十公里、能量在 MeV 量级），物质势 $V_e$ 的数值通常较小，仅作为微小却必要的修正被纳入精密拟合，以避免在高精度测量（如 JUNO 等）中产生系统偏差。
