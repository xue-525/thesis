...
由公式~\ref{eq:flavor-mass-superposition} 可以推导出能量为 $E$ 的中微子传播距离 $L$ 以后的概率为：
...
对于中微子混合矩阵（PMNS 矩阵），中微子混合矩阵 $U_{\mathrm{PMNS}}$ 将味态和质量态联系起来，是一个 $3\times3$ 酉矩阵。一般可用三个混合角 $(\theta_{12},\theta_{23},\theta_{13})$ 和一个狄拉克 CP 破坏相位 $\delta$ 来参数化（如果中微子是马约拉纳粒子，还可多出两个马约拉纳相位 $\alpha_{21},\alpha_{31}$）。一种常见的表示形式为：
$$
\begin{aligned}
...
由公式~\ref{eq:flavor-mass-superposition} 以及 PMNS 矩阵~\ref{eq:formula_prob} 可知，描述中微子振荡的参数有 6 个，分别是混合角 $\theta_{13}$、$\theta_{12}$、$\theta_{23}$，质量平方差 $\Delta m_{21}^2$ 和 $\Delta m_{32}^2$，CP 破坏相角 $\delta$。而 JUNO 实验将给出振荡参数的精确测量 $(\theta_{12},\Delta m_{21}^2)$ 以及 $\Delta m_{32}^2$ 的正负号（质量顺序）。
