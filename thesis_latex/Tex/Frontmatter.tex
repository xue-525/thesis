...
中微子作为连接粒子物理与宇宙学的关键“信使”，其非零质量与振荡现象提供了超越标准模型的新物理证据。当前，中微子研究已从现象发现迈向精密测量时代。虽然多数振荡参数已得到较好约束，但判定中微子质量顺序（MO）与测量 CP 破坏相位仍是核心挑战。解决这些关键问题不仅有助于完善大统一理论，更是揭示宇宙物质-反物质不对称性及演化奥秘的关键突破口。

江门中微子实验（JUNO）依托地下 700 米处的 20 千吨液体闪烁体探测器，通过对 52.5 公里基线处的反应堆反中微子进行测量，旨在判定中微子质量序并实现振荡参数的亚百分比量级测量。早期仅 59.1 天的运行数据便将太阳振荡参数的测量精度较全球既有结果提升了 1.6 倍，有力验证了探测器卓越的能量分辨率与大规模工程设计的可行性。然而在实验运行早期，如何在低统计量条件下减少拟合偏差、降低对反应堆模型依赖性并确保统计推断的稳健性，是JUNO实验物理分析的关键挑战。本文围绕反应堆中微子振荡分析中的物理链条，从数值算法优化、能谱分段策略与严格统计方法三个维度开展了深入研究。
...
其次，本文系统评估了重建能谱的分 bin 策略及反应堆能谱的分段（segment）方案。研究发现，在低统计量阶段，100 keV 以上的重建能谱 bin 宽能有效抑制由非高斯统计涨落引入的参数偏差。在 JUNO–大亚湾联合分析中，本文提出将反应堆能谱 segment 宽度设为 300 keV，这一策略成功在抑制模型依赖（系统误差贡献低于 0.3\%）与保留物理灵敏度之间取得了平衡。该方案已被 JUNO 首篇物理结果论文采纳。
...
\keywords{江门中微子实验（JUNO）；反应堆中微子；中微子振荡；高性能拟合；Feldman-Cousins 方法；分 bin 策略}% 中文关键词
%-
%-> 英文摘要
%-
\intobmk\chapter*{Abstract}% 显示在书签但不显示在目录

As crucial "messengers" connecting particle physics and cosmology, neutrinos, with their non-zero mass and oscillation phenomena, provide evidence for new physics beyond the Standard Model. Neutrino research has transitioned from the era of discovery to an era of precision measurement. While most oscillation parameters are now well-constrained, determining the Neutrino Mass Ordering (MO) and measuring the CP-violating phase remain core challenges. Resolving these issues is fundamental to refining Grand Unified Theories and uncovering the mysteries of the universe's evolution and its matter-antimatter asymmetry.

The Jiangmen Underground Neutrino Observatory (JUNO), featuring a 20-kiloton liquid scintillator detector situated 700 meters underground, is designed to determine the MO and achieve sub-percent precision in oscillation parameter measurements by detecting reactor antineutrinos at a 52.5 km baseline. With only 59.1 days of early operation data, JUNO has already improved the precision of solar oscillation parameters by a factor of 1.6 compared to existing global results, successfully validating the detector’s superior energy resolution and the feasibility of its large-scale engineering design. However, reducing fitting biases under low-statistics conditions, minimizing reactor model dependence, and ensuring the robustness of statistical inference remain critical challenges for JUNO’s early-stage physical analysis. This thesis conducts an in-depth study along the reactor neutrino oscillation analysis chain, focusing on three dimensions: numerical algorithm optimization, energy binning strategies, and rigorous statistical methods.
...
\KEYWORDS{Jiangmen Underground Neutrino Observatory (JUNO); Reactor Neutrinos; Neutrino Oscillation; High-Performance Fitting; Feldman-Cousins Method; Binning Strategy}% 英文关键词
